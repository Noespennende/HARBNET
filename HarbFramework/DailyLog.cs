...
namespace Gruppe8.HarbNet
{
    ...
    public class DailyLog : HistoryRecord
    {
            ...
            IList<Ship> shipsDockedInShipDocks)
        {
            ...
            ShipsInAnchorage = new ReadOnlyCollection<Ship>(DuplicateShipList(shipsInAnchorage));

            ShipsInTransit = new ReadOnlyCollection<Ship>(DuplicateShipList(shipsInTransit));

            ShipsDockedInLoadingDocks = new ReadOnlyCollection<Ship>(DuplicateShipList(shipsDockedInLoadingDocks));

            ShipsDockedInShipDocks = new ReadOnlyCollection<Ship>(DuplicateShipList(shipsDockedInShipDocks));
